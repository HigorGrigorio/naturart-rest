...
export abstract class AbstractService<M extends Model> implements IService<M> {
     ...
     */
    public async add(attributes: CreationAttributes<M>): Promise<NaturartResponse<M>> {
     ...
     */
    async deleteById(where: WhereOptions<Attributes<M>>): Promise<NaturartResponse<number>> {
     ...
     */
    async update(where: WhereOptions<Attributes<M>>, attributes: { [key in keyof Attributes<M>]?: Attributes<M>[key] }): Promise<NaturartResponse<number>> {
